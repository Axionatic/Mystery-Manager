...
def _exact_match(xlsx_name: str, db_parts: dict[str, dict]) -> dict | None:
    ...
    if xlsx_name in db_parts:
        return {"id": db_parts[xlsx_name]["item_id"], "db_name": xlsx_name, "method": "exact"}
    ...
    if len(matches) == 1:
        ...
        return {"id": info["item_id"], "db_name": db_name, "method": "prefix"}
...
def match_items(
    ...
    try:
        ...
        if not db_parts:
            logger.info(f"Offer {offer_id}: no live items found, retrying with soft-deleted records")
    ...
    if llm_needed:
        ...
        def _match_one(xlsx_name: str) -> tuple[str, dict | None]:
            ...
            if result:
                result["id"] = db_parts[result["db_name"]]["item_id"]
    ...
    if price_column:
        for xlsx_name, mapping in cache.items():
            ...
            if db_name in db_parts:
                db_price = db_parts[db_name]["item_price"]

...
_TIER_A_EXAMPLE = """\
,Mediums,,Smalls,,,,,,,,Charity
ID,alice@example.com,Md Alex,bob@example.com,charlie@example.edu.au,dana@example.com,evan@example.net.au,fiona@example.com,owner@example.shop,Sm Pat M,Sm Lee H,Charity
5050,,,,,,,,,,,61
5089,4,4,4,4,4,4,4,6,20,,
4966,4,4,2,2,2,2,2,,20,,
4986,2,2,2,2,2,2,2,2,,10,10
4978,4,4,2,2,2,2,2,2,10,,
5090,,3,2,2,2,2,2,2,,10,
5004,,2,1,1,1,1,1,,,,15
5153,2,2,2,2,2,2,2,2,5,,"""


def write_sheet_csv(ws, csv_path: Path, max_rows: int = 100) -> str:
    """
    Write a worksheet to a CSV file and return the content as a string.
    ...
    Returns:
        The CSV content as a string (for prompt embedding).
    """
    if ws.max_row is None or ws.max_row < 1:
        csv_path.write_text("(empty sheet)")
        return "(empty sheet)"
...
Key features:
- The second row has column headers: "ID" column (DB item IDs), then one column per customer box (email or name), then {CHARITY_NAME} (charity)
- The first row may have size grouping labels ("Mediums", "Smalls", "{CHARITY_NAME}")
...
def analyze_and_extract(
    ...
    # Read all sheets, writing CSV files to a temp dir
    wb = openpyxl.load_workbook(xlsx_path, data_only=True)
    csv_dir = tempfile.mkdtemp(prefix="mystery_extract_")
    ...
    try:
        ...
        for sn in wb.sheetnames:
            ...
            if ws.max_row is None or ws.max_row < 2:
                ...
                continue
            csv_path = Path(csv_dir) / f"{sn}.csv"
